...
////////////////////////////
	...
	//BUAT NEGATIF
	public void searchNamaProgram(String namaPrg) {
        this.searchField.sendKeys(namaPrg);
        try {
            Robot rbt = new Robot();
            rbt.keyPress(KeyEvent.VK_ENTER);
            rbt.keyRelease(KeyEvent.VK_ENTER);
	...
	public void searchHargaNormal(String hrgNrml) {
        this.searchField.sendKeys(hrgNrml);
        try {
            Robot rbt = new Robot();
            rbt.keyPress(KeyEvent.VK_ENTER);
            rbt.keyRelease(KeyEvent.VK_ENTER);
    ...
    public void searchDiskon(String dsc) {
        this.searchField.sendKeys(dsc);
        try {
            Robot rbt = new Robot();
            rbt.keyPress(KeyEvent.VK_ENTER);
            rbt.keyRelease(KeyEvent.VK_ENTER);
  ...
  //BUAT NEGATIF
	...
	public void tambahDataRincianBiayaPublish(String status) {
		...
		this.harga.sendKeys("1000000");
		this.diskon.sendKeys("10");
		this.keunggulan1.sendKeys("bisa sampai mahir awal");
		this.keunggulan2.sendKeys("keunggulan keempat awal");
		this.keunggulan3.sendKeys("keunggulan ketiga awal");
		this.keunggulan4.sendKeys("keunggulan awal");
		Select selPublish = new Select(this.optPublish);
		selPublish.selectByVisibleText(status);
		Utils.delay(2, strDelay);
		Utils.fullScroll();
	}
	public void tambahNegative(String hargaAwal, String diskon, String publish ) {
        if (!hargaAwal.isBlank()) {
            this.harga.sendKeys(hargaAwal);
            this.nama.sendKeys(namaTambah);
            this.diskon.sendKeys("20");
            this.keunggulan1.sendKeys("bisa sampai mahir manual");
            this.keunggulan2.sendKeys("keunggulan keempat manual");
            this.keunggulan3.sendKeys("keunggulan ketiga manual");
            this.keunggulan4.sendKeys("keunggulan manual");
            Select selPublish = new Select(this.optPublish);
            selPublish.selectByVisibleText("Active");
            Utils.delay(2, strDelay);
            Utils.fullScroll();
        }else if(!diskon.isBlank()) {
            this.nama.sendKeys(namaTambah);
            ...
            this.keunggulan4.sendKeys("keunggulan");
            Select selPublish = new Select(this.optPublish);
            selPublish.selectByVisibleText(publish);
            Utils.delay(2, strDelay);
            Utils.fullScroll();
        }
	    this.harga.sendKeys(hargaAwal);
	...
	private WebElement warningNameEmpty;
	public String getWarningNameEmpty() {
        //"Data berhasil di tambah"
        return new WebDriverWait(driver, Duration.ofSeconds(15))
                .until(ExpectedConditions.visibilityOf(warningNameEmpty)).getText();
    }
	@FindBy(xpath ="//div[5]//div[1]//div[1]")
	private WebElement warningKeunggulan1;
	public String getWarningKeunggulan1Empty() {
        //"Data berhasil di tambah"
        return new WebDriverWait(driver, Duration.ofSeconds(15))
                .until(ExpectedConditions.visibilityOf(warningKeunggulan1)).getText();
    }
